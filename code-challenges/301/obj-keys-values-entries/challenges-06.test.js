...
const hasChildrenValues = (arr, character) => {
  let result= '';
  arr.forEach(value => { 
    let charObj = Object.values(value);
...
const hasChildrenEntries = (arr, character) => {
  let result= ''; 
  arr.forEach(value => { 
    let charObj = Object.entries(value);
     if(charObj[0][1]===character){
       if(charObj[2][1].length>0){
         result = true;
       }
       else{ 
         result = false
...
const houseSize = (arr) => {
  const sizes = [];
  let houses = [];
  //foreach creating houses array
  arr.forEach(obj => {
    let charObj = Object.values(value);
    if (houses.includes(charObj[3])){}
    else{ houses.push(charObj[3])}
  });
  return sizes;
}
