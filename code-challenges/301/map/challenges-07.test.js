...
const mapTwoToThe = (arr) => {
  return arr.map( (value, idx) => Math.pow(2, value) );
}
...
const charCode = (arr) => {
  return arr.map((value, idx) => value.charCodeAt(0));
};
...
const extractAbilities = (arr) => {
  return arr.map((value, idx) => value.ability.name)
};
