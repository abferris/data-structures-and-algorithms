...
const firstLetters = (arr) => {
  let firsts = [];
  arr.forEach(str => firsts.push(str.charAt(0)));
  return firsts;
};
...
const findHappiness = (arr) => {
  let faces = [];
  arr.forEach(str => str.includes(':)') ? faces.push(str): 0);
  return faces;
};
...
const standardizePhoneNumbers = (arr) => {
  let ans= []  
  arr.forEach(str => {
    ans.push([str.substring(1,4), str.substring(6,9), str.substring(10,15)].join(''))
  });
  return ans
};
